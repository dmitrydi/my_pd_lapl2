...
RealWell::RealWell(const double xwd, const double xed, const double ywd, const double yed, const double Fcd): stehf_coefs(CalcStehf(NCOEF)),
		lwell(LaplWell(xwd,xed,ywd,yed,Fcd)){
};
double RealWell::pd(const double td) {
	double s_mult = std::log(2.)/td;
	double ans = 0.;
	for (int i = 1; i <=NCOEF; ++i) {
		double s = i*s_mult;
		ans += lwell.pd(s)*s*stehf_coefs[i]/i;
	}
	return ans;
};
