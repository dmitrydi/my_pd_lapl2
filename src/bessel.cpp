...
__float128 ibessk0q(const double xd) {
	__float128 x = xd;
	if (x < 0.0q) throw std::invalid_argument("in ibessk0q: x < 0");
	if (x < TINY_DBL) return 0.0q;
	if (x >= 36.0q) return PIo2Q;
	__float128 xo2 = 0.5q*static_cast<__float128>(x);
	const __float128 A = -(logq(xo2) + EUL_GAMMA_Q);
	Compensated::NeumaierQuad summator;
	Compensated::NeumaierQuad n_summator;
	__float128 mem1, mem2, mem3 = 1.0q;
	summator.Add(A);
	summator.Add(1.0q);
	__float128 dum = 1.q;
	__float128 aux, denom, _mem;
	__float128 k128;
	int64_t k = 0;
	while (MAXMULT*mem3 > EPS_QUAD*(x*summator.Get())) {
		++k;
		k128 = static_cast<__float128>(k);
		aux = xo2/k128;
		aux *= aux;
		dum *= aux;
		denom = (2.*k128 + 1.0q);
		...
		mem1 = A*_mem;
		mem2 = _mem/denom;
		n_summator.Add(1./k128);
		mem3 = _mem*n_summator.Get();
		summator.Add(mem1);
		summator.Add(mem2);
		summator.Add(mem3);
	}
	return x*summator.Get();
}

double ibessk0d(const double x){
	double xo2 = 0.5*x;
	const double A = -(std::log(xo2) + EUL_GAMMA);
	Compensated::Neumaier<double> summator;
	Compensated::Neumaier<double> n_summator;
	double mem1, mem2, mem3 = 1.;
	summator.Add(A);
	summator.Add(1.);
	double dum = 1.;
	double aux, denom, _mem;
	int64_t k = 0;
	while (mem3 > TINY_DBL){//
		++k;
		aux = xo2/k;
		...
		n_summator.Add(1./k);
		mem3 = _mem*n_summator.Get();
		if (std::isinf(mem3) || std::isinf(mem2)||std::isinf(mem1)) break;
		summator.Add(mem1);
		summator.Add(mem2);
		summator.Add(mem3);
	}
...
double ibessk0(const double x) {
	//based on ./src/test_results/Precision_f.txt
	if (x <= 6.0) return ibessk0d(x);
	return static_cast<double>(ibessk0q(x));
}

double ibessk0ab_d(const double a, const double b) {
	return ibessk0d(b) - ibessk0d(a);
}
__float128 ibessk0ab_q(const double a, const double b) {
	return ibessk0q(b) - ibessk0q(a);
}
